...
class ChecklistProgressRequestMapper @Inject constructor() {
    ...
    fun buildChecklistProgressRequest(
        ...
        overallNotes: String = "Trabajo completado parcialmente"
    ): JSONObject {
        ...
        mainJson.put("auto_calculate_progress", true)

        // ===== CHECKLIST_UPDATES WRAPPER =====
        val checklistUpdates = JSONObject()

        // Extraer el nombre del template
        val templateName = templateData.getString("name")
        checklistUpdates.put("name", templateName)
        ...
        checklistUpdates.put("sections", sectionsArray)
        ...
        if (templateData.has("serviceFields")) {
            ...
            checklistUpdates.put("serviceFields", fieldsArrayResponse)
        }

        // ===== AGREGAR CHECKLIST_UPDATES AL JSON PRINCIPAL =====
        mainJson.put("checklist_updates", checklistUpdates)
        mainJson.put("overall_notes", overallNotes)
        return mainJson
    }
}
